        ...
        foreach ($this->dummy as $productData) {
            ...
            foreach ($productData['urls'] as $url => $urlData) {
                ...
                if (is_array($urlData) && array_key_exists('prices', $urlData)) {
                    $factory = $factory->addUrlWithPrices($url, $urlData['prices'], $urlData['availability'] ?? null, $priceFactor);
                } else {
                    $factory = $factory->addUrlWithPrices($url, $urlData, null, $priceFactor);
                }
            }
